...
for ((i=0; i<$NUM_NODES; i++)) do
    ssh $PEM_FLAG ubuntu@${IPS[$i]} "git clone https://github.com/0glabs/0g-evmos.git; cd 0g-evmos; ./networks/testnet/install.sh"
done

# Create genesis config on node0
ssh $PEM_FLAG ubuntu@${IPS[0]} "cd 0g-evmos/networks/testnet; $INIT_GENESIS_ENV ./init-genesis.sh $IP_LIST $KEYRING_PASSWORD; tar czf ~/$NETWORK.tar.gz $NETWORK; rm -rf $NETWORK"
...
for ((i=0; i<$NUM_NODES; i++)) do
    ...
    ssh $PEM_FLAG ubuntu@${IPS[$i]} "rm -rf evmosd-prod; tar xzf node$i.tar.gz; rm node$i.tar.gz; mv node$i evmosd-prod"
    rm node$i.tar.gz
done
